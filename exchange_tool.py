...
def get_exchange_rate(from_currency: str, to_currency: str, query: str) -> dict:
...
def print_exchange_rate_info(result):
...
def main():
    ...
    try:
        # Parse the query
        query_parts = args.query.upper().split(" TO ")
        if len(query_parts) != 2:
            print("Error: Invalid query format. Use 'CURRENCY1 to CURRENCY2 [last X months/years]' or 'CURRENCY1 to CURRENCY2 YYYY-MM-DD~YYYY-MM-DD'")
            return
        ...
        result = get_exchange_rate(from_currency, to_currency, args.query)
        print_exchange_rate_info(result)
    except Exception as e:
        print(f"Error: {e}")
